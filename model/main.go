...
func migrateDB() error {
	...
	err := DB.AutoMigrate(
		...
		&CasbinRule{},
		&AuthzRole{},
		&TokenDailyModelSummary{},
		&TokenDailyTotalSummary{},
	)
	if err != nil {
		return err
	}
...
func migrateLOGDB() error {
	...
	return LOG_DB.AutoMigrate(&Log{}, &LogHistory{})
}
...
func backfillTokenDailySummaries() {
	var count int64
	if err := DB.Model(&TokenDailyModelSummary{}).Count(&count).Error; err != nil {
		common.SysLog("Warning: failed to check token_daily_model_summary: " + err.Error())
		return
	}
	if count > 0 {
		common.SysLog("token_daily_model_summary already has data, skipping backfill")
		return
	}
	common.SysLog("backfilling token daily summary tables from logs...")
	if common.UsingMainDatabase(common.DatabaseTypeMySQL) {
		DB.Exec(
			"INSERT IGNORE INTO token_daily_model_summary " +
			"(stat_date, token_id, token_key, token_name, token_group, model_name, " +
			"request_count, total_quota, total_prompt_tokens, total_completion_tokens, total_tokens, " +
			"first_request_at, last_request_at) " +
			"SELECT " +
			"DATE_FORMAT(FROM_UNIXTIME(l.created_at), '%Y-%m-%d') AS stat_date, " +
			"COALESCE(l.token_id, 0) AS token_id, " +
			"COALESCE(t.key, '') AS token_key, " +
			"COALESCE(l.token_name, '') AS token_name, " +
			"COALESCE(t.`group`, l.`group`, '') AS token_group, " +
			...
			"WHERE l.type = 2 " +
			"GROUP BY stat_date, l.token_id, l.token_name, t.key, t.`group`, l.`group`, l.model_name",
		)
		DB.Exec(
			"INSERT IGNORE INTO token_daily_total_summary " +
			"(stat_date, token_id, token_key, token_name, token_group, models_used, distinct_models, " +
			"total_requests, total_quota, total_prompt_tokens, total_completion_tokens, total_tokens, " +
			"day_first_request, day_last_request) " +
			"SELECT " +
			"DATE_FORMAT(FROM_UNIXTIME(l.created_at), '%Y-%m-%d') AS stat_date, " +
			"COALESCE(l.token_id, 0) AS token_id, " +
			"COALESCE(t.key, '') AS token_key, " +
			"COALESCE(l.token_name, '') AS token_name, " +
			"COALESCE(t.`group`, l.`group`, '') AS token_group, " +
			"GROUP_CONCAT(DISTINCT l.model_name ORDER BY l.model_name ASC SEPARATOR ', ') AS models_used, " +
			...
			"LEFT JOIN tokens t ON l.token_id = t.id " +
			"WHERE l.type = 2 " +
			"GROUP BY stat_date, l.token_id, l.token_name, t.key, t.`group`, l.`group`",
		)
	} else if (common.UsingMainDatabase(common.DatabaseTypePostgreSQL)) {
			DB.Exec(
				"INSERT INTO token_daily_model_summary " +
				"(stat_date, token_id, token_key, token_name, token_group, model_name, " +
				"request_count, total_quota, total_prompt_tokens, total_completion_tokens, total_tokens, " +
				"first_request_at, last_request_at) " +
				"SELECT " +
				"TO_CHAR(TO_TIMESTAMP(l.created_at), 'YYYY-MM-DD') AS stat_date, " +
				"COALESCE(l.token_id, 0) AS token_id, " +
				"COALESCE(t.key, '') AS token_key, " +
				"COALESCE(l.token_name, '') AS token_name, " +
				"COALESCE(t.\"group\", l.\"group\", '') AS token_group, " +
				...
				"COALESCE(SUM(l.completion_tokens), 0) AS total_completion_tokens, " +
				"COALESCE(SUM(l.prompt_tokens + l.completion_tokens), 0) AS total_tokens, " +
				"TO_TIMESTAMP(MIN(l.created_at))::text AS first_request_at, " +
				"TO_TIMESTAMP(MAX(l.created_at))::text AS last_request_at " +
				"FROM logs l " +
				"LEFT JOIN tokens t ON l.token_id = t.id " +
				"WHERE l.type = 2 " +
				"GROUP BY stat_date, l.token_id, l.token_name, t.key, t.\"group\", l.\"group\", l.model_name " +
				"ON CONFLICT (stat_date, token_id, model_name) DO NOTHING",
			)
			DB.Exec(
				"INSERT INTO token_daily_total_summary " +
				"(stat_date, token_id, token_key, token_name, token_group, models_used, distinct_models, " +
				"total_requests, total_quota, total_prompt_tokens, total_completion_tokens, total_tokens, " +
				"day_first_request, day_last_request) " +
				"SELECT " +
				...
				"COALESCE(l.token_name, '') AS token_name, " +
				"COALESCE(t.\"group\", l.\"group\", '') AS token_group, " +
				"STRING_AGG(DISTINCT l.model_name, ', ' ORDER BY l.model_name) AS models_used, " +
				"COUNT(DISTINCT l.model_name) AS distinct_models, " +
				"COUNT(*) AS total_requests, " +
				"COALESCE(SUM(l.quota), 0) AS total_quota, " +
				"COALESCE(SUM(l.prompt_tokens), 0) AS total_prompt_tokens, " +
				"COALESCE(SUM(l.completion_tokens), 0) AS total_completion_tokens, " +
				"COALESCE(SUM(l.prompt_tokens + l.completion_tokens), 0) AS total_tokens, " +
				"TO_TIMESTAMP(MIN(l.created_at))::text AS day_first_request, " +
				"TO_TIMESTAMP(MAX(l.created_at))::text AS day_last_request " +
				"FROM logs l " +
				"LEFT JOIN tokens t ON l.token_id = t.id " +
				"WHERE l.type = 2 " +
				"GROUP BY stat_date, l.token_id, l.token_name, t.key, t.\"group\", l.\"group\" " +
				"ON CONFLICT (stat_date, token_id) DO NOTHING",
			)
		} else {
			// SQLite
			DB.Exec(
				"INSERT OR IGNORE INTO token_daily_model_summary " +
				"(stat_date, token_id, token_key, token_name, token_group, model_name, " +
				"request_count, total_quota, total_prompt_tokens, total_completion_tokens, total_tokens, " +
				"first_request_at, last_request_at) " +
				"SELECT " +
				"date(l.created_at, 'unixepoch') AS stat_date, " +
				"COALESCE(l.token_id, 0) AS token_id, " +
				"COALESCE(t.key, '') AS token_key, " +
				"COALESCE(l.token_name, '') AS token_name, " +
				"COALESCE(t.\"group\", l.\"group\", '') AS token_group, " +
				...
				"COALESCE(SUM(l.completion_tokens), 0) AS total_completion_tokens, " +
				"COALESCE(SUM(l.prompt_tokens + l.completion_tokens), 0) AS total_tokens, " +
				"datetime(MIN(l.created_at), 'unixepoch') AS first_request_at, " +
				"datetime(MAX(l.created_at), 'unixepoch') AS last_request_at " +
				"FROM logs l " +
				"LEFT JOIN tokens t ON l.token_id = t.id " +
				"WHERE l.type = 2 " +
				"GROUP BY stat_date, l.token_id, l.token_name, t.key, t.\"group\", l.\"group\", l.model_name",
			...
			DB.Exec(
				"INSERT OR IGNORE INTO token_daily_total_summary " +
				"(stat_date, token_id, token_key, token_name, token_group, models_used, distinct_models, " +
				"total_requests, total_quota, total_prompt_tokens, total_completion_tokens, total_tokens, " +
				"day_first_request, day_last_request) " +
				"SELECT " +
				...
				"COALESCE(l.token_name, '') AS token_name, " +
				"COALESCE(t.\"group\", l.\"group\", '') AS token_group, " +
				"GROUP_CONCAT(DISTINCT l.model_name) AS models_used, " +
				"COUNT(DISTINCT l.model_name) AS distinct_models, " +
				"COUNT(*) AS total_requests, " +
				"COALESCE(SUM(l.quota), 0) AS total_quota, " +
				"COALESCE(SUM(l.prompt_tokens), 0) AS total_prompt_tokens, " +
				"COALESCE(SUM(l.completion_tokens), 0) AS total_completion_tokens, " +
				"COALESCE(SUM(l.prompt_tokens + l.completion_tokens), 0) AS total_tokens, " +
				"datetime(MIN(l.created_at), 'unixepoch') AS day_first_request, " +
				"datetime(MAX(l.created_at), 'unixepoch') AS day_last_request " +
				"FROM logs l " +
				"LEFT JOIN tokens t ON l.token_id = t.id " +
				"WHERE l.type = 2 " +
				"GROUP BY stat_date, l.token_id, l.token_name, t.key, t.\"group\", l.\"group\"",
	...
	common.SysLog("token daily summary tables backfill completed")
}
